...
All v1 tools plus v2 enhancements for Airflow 3.0+ support.
"""
import os
import logging
from typing import Any, Dict, List, Optional

from ..functions import airflow_request_v2 as airflow_request, read_prompt_template, parse_prompt_sections, get_current_time_context, list_dags_internal, get_dag_detailed_info, PROMPT_TEMPLATE_PATH

# Setup logging
logger = logging.getLogger(__name__)

def register_tools(mcp):
    """Register all v1 tools with the MCP server."""
    
    # All extracted tool functions
    @mcp.tool()
    ...
    async def list_asset_events(limit: int = 20, offset: int = 0, 
        ...
        resp = await airflow_request("GET", f"/assets/events?{query_string}")
        resp.raise_for_status()
        data = resp.json()
    ...
    async def list_assets(limit: int = 20, offset: int = 0,
        ...
        """
        [V2 New] List all assets in the system.
        ...
        resp = await airflow_request("GET", f"/assets?{query_string}")
        resp.raise_for_status()
        data = resp.json()
