"""
Airflow API v2 (3.0+) MCP tools with enhanced features.

- Airflow API Documents: https://airflow.apache.org/docs/apache-airflow/stable/stable-rest-api-ref.html

This module includes all v1 tools with v2 enhancements plus new v2-only features.
"""
import os
import logging
from typing import Any, Dict, List, Optional

from ..functions import airflow_request_v2 as airflow_request, read_prompt_template, parse_prompt_sections, get_current_time_context, list_dags_internal, get_dag_detailed_info
from .common import PROMPT_TEMPLATE_PATH
...
def register_tools(mcp):
    ...
    async def list_asset_events(limit: int = 20, offset: int = 0,
        ...
        """
        [V2 New] List asset/dataset events (new in Airflow 3.0).
        
        Assets (formerly Datasets) are a new concept in Airflow 3.0 for data-aware scheduling.
        """
        ...
        resp = await airflow_request("GET", f"/datasets/events?{query_string}")
        resp.raise_for_status()
        data = resp.json()
        
        return {
        "asset_events": data.get("dataset_events", []),
        ...
        @mcp.tool()
    async def list_datasets(limit: int = 20, offset: int = 0,
        uri_pattern: Optional[str] = None) -> Dict[str, Any]:
        """
        [V2 New] List all datasets/assets in the system.
        
        Datasets are a key feature in Airflow 3.0 for data-aware scheduling.
        """
        params = {'limit': limit, 'offset': offset}
        if uri_pattern:
        ...
        resp = await airflow_request("GET", f"/datasets?{query_string}")
        resp.raise_for_status()
        data = resp.json()
        ...
        logger.info("Registered all Airflow API v2 tools (v1 + v2 enhancements)")
